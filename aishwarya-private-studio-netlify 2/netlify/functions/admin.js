...
async function customerUpdate(body, event) { const account = customerSession(event); const customers = await storeList(CUSTOMERS); const index = customers.findIndex(customer => customer.id === account.sub); if (index < 0) throw fail(401, 'Please sign in again.'); const name = clean(body.name), phone = normalPhone(body.phone); if (name.length < 2 || name.length > 80) throw fail(400, 'Enter your full name.'); if (!/^\+?[0-9]{8,16}$/.test(phone)) throw fail(400, 'Enter a valid mobile number including the country code.'); if (customers.some((customer, i) => i !== index && customer.phone === phone)) throw fail(409, 'Another account already uses this mobile number.'); customers[index] = { ...customers[index], name, phone, updatedAt: new Date().toISOString() }; await put(CUSTOMERS, customers); return { customer: { name, email: customers[index].email, phone } }; }
...
async function customerOrders(event) { const account = customerSession(event); const customer = (await storeList(CUSTOMERS)).find(item => item.id === account.sub); return { orders: (await storeList(ORDERS)).filter(order => order.customerId === account.sub || (customer && order.customer && order.customer.email === customer.email)).map(publicOrder) }; }
...
async function saveProduct(body, event) { ownerSession(event); const items = await storeList(PRODUCTS); const name = clean(body.name); const price = Number(body.price); const stock = Number(body.stock);
  ...
  const item = { id: clean(body.id) || productId(), name, category: clean(body.category) || 'Uncategorised', price, stock, description: clean(body.description), fabric: clean(body.fabric), colour: clean(body.colour), blouseDetails: clean(body.blouseDetails), length: clean(body.length), care: clean(body.care), deliveryEstimate: clean(body.deliveryEstimate), image: image || clean(body.existingImage), active: body.active !== false, updatedAt: new Date().toISOString() };
...
exports.handler = async event => {
  ...
  try {
    ...
    if (action === 'customer/update') return json(200, await customerUpdate(body, event));
